...
public class UserController {
    ...
    public ApiResponse<?> getUserList(
        ...
        try {
            ...
            if (FeignResponseUtil.isSuccess(response, logger, "获取用户列表")) {
    ...
    public ApiResponse<?> getUserDetail(@RequestParam Long userId) {
        try {
    ...
    public ApiResponse<?> disableUser(@PathVariable Long userId) {
        try {
            ...
            logger.info("用户服务禁用用户响应: {}", response);
            
            // 增强响应判断逻辑：检查响应是否成功，同时考虑code为200的情况
            if (response != null && response.getCode() == 200) {
                logger.info("禁用用户成功，userId: {}", userId);
                return ApiResponse.success(response.getData());
            } else {
    ...
    public ApiResponse<?> enableUser(@PathVariable Long userId) {
        try {
            ...
            logger.info("用户服务启用用户响应: {}", response);
            
            // 增强响应判断逻辑：检查响应是否成功，同时考虑code为200的情况
            if (response != null && response.getCode() == 200) {
                logger.info("启用用户成功，userId: {}", userId);
                return ApiResponse.success(response.getData());
            } else {
    ...
    @Operation(summary = "获取用户订阅列表", description = "分页获取用户的订阅信息，支持筛选")
