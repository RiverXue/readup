...
目前系统共包含 **50+个** REST API接口，分布如下：

- **AI服务**：12个接口（DeepSeek智能分析、腾讯云翻译、AI助手、读后测验持久化、AI学习助手）
- **文章服务**：7个接口（文章发现、阅读管理、内容提取）
- **用户服务**：18个接口（用户认证、多级词库、订阅管理）
- **报告服务**：9个接口（学习统计、艾宾浩斯复习、数据仪表盘、用户数据分析）
- **管理员服务**：25个接口（系统配置管理、后台管理、数据统计）

> **📝 更新说明**：经过AI API清理优化，删除了10个冗余接口，保留核心功能，提升系统性能和维护性。新增AI学习助手和用户数据分析功能。
...
## 🧠 AI 服务 (ai-service) - 12个接口

提供智能分析、翻译和AI助手功能。服务采用双引擎策略：DeepSeek（进阶AI）+ 腾讯云（基础翻译）。

> **🚀 最新优化**：完成AI API清理，删除冗余接口，新增读后测验持久化功能，提升系统性能和用户体验。
...
- **业务逻辑**：使用DeepSeek Structured Outputs生成标准化题目，支持多种题型，结果自动持久化
...
```http
GET /api/ai/assistant/quiz/{articleId}
```

- **使用场景**：获取指定文章已保存的测验题，优先加载缓存数据
- **业务逻辑**：
  - 优先从数据库加载已保存的测验题
  - 如果没有保存的测验题，返回空列表
  - 支持测验题数据的持久化存储
...
- **响应**：`ApiResponse<AiChatResponse>`
  ...
  ```json
  {
    ...
    "success": true,
    "data": {
      "answer": "Rayda老师的个性化回答...",
      "suggestions": ["具体学习建议1", "具体学习建议2"],
      "followUpQuestions": ["相关问题1", "相关问题2"],
      "learningTips": "学习技巧和提示"
    }
  }
  ```
...
```http
POST /api/ai/assistant/profile-analysis
```

- **使用场景**：分析用户学习数据，生成学习画像和诊断报告
- **业务逻辑**：
  - 多维度分析用户学习行为数据
  - 识别学习薄弱环节和优势能力
  - 生成个性化学习建议
- **请求体**：`UserProfileAnalysisRequest`
  ...
  ```
- **响应**：`ApiResponse<UserProfileAnalysis>`
  ...
  ```json
  {
    ...
    "data": {
      "currentLevel": "intermediate",
      "weakAreas": ["阅读速度", "词汇积累", "学习习惯"],
      "strengths": ["学习坚持性", "词汇掌握"],
      "recommendations": [
        "建议每天固定时间学习，建立学习习惯",
        "建议每周阅读2-3篇文章，提高阅读理解能力",
        "建议每天学习10-15个新单词，扩大词汇量"
      ],
      "learningProgress": {
        "levelProgress": 65,
        "nextLevel": "advanced",
...
> **使用场景**：项目核心功能，采用"本地缓存 + AI增强"策略，实现智能词汇管理。
...
- **使用场景**：智能词汇系统：本地缓存优先，AI增强生成，支持词汇管理
- **业务逻辑**：
  - 首先查询本地词汇缓存
  - 未命中则查询数据库词汇
  - 都未命中则调用AI服务生成
...
### v2.3.0 (2025-10-17) - 当前版本

- 🧹 **API清理**：完成AI API全面清理，删除10个冗余接口
- ✨ **新增功能**：读后测验数据持久化，优先加载已保存的测验题
- 🎨 **UI优化**：添加读后测验按钮加载动画，提升用户体验
- 🚀 **性能优化**：减少AI调用成本，提升系统响应速度
- 📝 **文档更新**：完善API文档，反映最新接口变更
