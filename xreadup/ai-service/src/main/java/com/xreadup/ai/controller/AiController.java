...
public class AiController {
    ...
    public ApiResponse<ArticleAnalysisResponse> analyzeArticle(@RequestBody ArticleAnalysisRequest request) {
        try {
            // 检查AI文章分析功能是否启用
            if (!aiConfigService.isAiArticleAnalysisEnabled()) {
                log.warn("AI文章分析功能已禁用，拒绝分析请求: {}", request.getArticleId());
                return ApiResponse.error("AI文章分析功能当前不可用");
            }
            
            // 检查系统维护模式
            if (aiConfigService.isMaintenanceMode()) {
                log.warn("系统处于维护模式，拒绝分析请求: {}", request.getArticleId());
