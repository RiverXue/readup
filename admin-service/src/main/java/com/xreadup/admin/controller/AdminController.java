...
import com.xreadup.admin.dto.PageResult;
...
public class AdminController {
    ...
    public ApiResponse<AdminUserDTO> getAdminUserDetail(@RequestParam("userId") Long userId) {
        try {
            AdminUserDTO adminUserDTO = adminUserService.getAdminUserByUserId(userId);
            if (adminUserDTO == null) {
                return ApiResponse.fail(404, "管理员用户不存在");
            }
            return ApiResponse.success(adminUserDTO);
        } catch (Exception e) {
    ...
    public ApiResponse<PageResult<AdminUserDTO>> getAdminUserList(
        ...
        try {
            ...
            PageResult<AdminUserDTO> pageResult = new PageResult<>(page, pageSize, total, adminUsers);
     ...
     */
    @PutMapping("/update")
    @Operation(summary = "更新管理员用户", description = "更新管理员用户的角色信息")
    public ApiResponse<Void> updateAdminUser(
            @RequestParam Long userId,
            @RequestParam String role) {
        try {
            adminUserService.updateAdminUser(userId, role);
            return ApiResponse.success();
        } catch (RuntimeException e) {
