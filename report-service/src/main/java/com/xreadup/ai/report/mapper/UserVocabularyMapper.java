...
public interface UserVocabularyMapper extends BaseMapper<UserVocabulary> {
    ...
    @Select("SELECT w.id as wordId, w.word, w.meaning, '' as example, w.review_status as difficulty, 0 as reviewCount, w.next_review_at as dueDate FROM word w WHERE w.user_ids LIKE CONCAT('%', #{userId}, '%') AND w.next_review_at <= CURDATE() ORDER BY w.next_review_at")
